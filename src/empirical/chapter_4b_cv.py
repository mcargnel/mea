...
def dml_est(df: pd.DataFrame, dep_vars: list, covariates: list) -> tuple[pd.DataFrame, dml.did.DoubleMLDIDMulti]: #type: ignore
    ...
    for dep_var in dep_vars:
        ...
        def rf_reg_params(trial):
            return {
                'n_estimators': trial.suggest_int('n_estimators', 50, 300, step=50),
                'max_depth': trial.suggest_int('max_depth', 3, 15)
            }

        def rf_clf_params(trial):
            return {
                'n_estimators': trial.suggest_int('n_estimators', 50, 300, step=50),
                'max_depth': trial.suggest_int('max_depth', 3, 15)
            }
        ...
        dml_did_obj.tune_ml_models(
            ml_param_space={'ml_g': rf_reg_params, 'ml_m': rf_clf_params},
            cv=2,
            optuna_settings={'n_trials': 20}
        )
        logger.info(f"Tuned ml_g and ml_m for {dep_var}")
