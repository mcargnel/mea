...
This chapter presents the Double Machine Learning framework introduced by \cite{Chernozhukov_2018}, which provides a rigorous method for combining machine learning flexibility with formal causal inference. We begin by establishing the core framework in a general partially linear model setting, demonstrating how the method addresses key challenges in estimating causal effects when controlling for high-dimensional covariates. We then show how this framework can be adapted to Difference-in-Differences settings, first in the canonical two-period case and subsequently in the more complex staggered adoption scenario.

The chapter is organized as follows. First, we introduce the general DML framework, starting with the estimation goal and the fundamental confounding problem. We then present the theoretical solution based on orthogonalization and explain how machine learning is used in practice to implement this solution. Second, we discuss the two critical techniques that ensure valid statistical inference despite using flexible machine learning methods: cross-fitting and Neyman orthogonality. Finally, we adapt this framework to Difference-in-Differences designs, showing how the core principles apply in both simple and staggered treatment timing contexts.

\section{The General Framework}

\subsection{Estimation Goal and Model Setup}

The fundamental objective in causal inference is to estimate the causal effect of a treatment $D$ on an outcome $Y$ while properly accounting for a potentially high-dimensional set of covariates $X$. We focus on estimating a constant treatment effect parameter $\theta$, which represents the Average Treatment Effect (ATE).

\begin{equation}
    ATE = E[Y_i(1)-Y_i(0)]
\end{equation}

Here, $Y_i(1)$ represents the potential outcome for unit $i$ under treatment, while $Y_i(0)$ represents the potential outcome without treatment. The fundamental challenge is that we observe only one of these potential outcomes for each unit, making direct estimation of the ATE impossible without additional structure.

To provide this structure, we adopt a Partially Linear Model specification, which forms the basis for the DML approach:
...
Specifically, we estimate $\hat{l}(X_i)$ as a machine learning approximation of $E[Y_i|X_i]$, typically framed as a regression task since outcomes are often continuous. Similarly, we estimate $\hat{m}(X_i)$ as an approximation of $E[D_i|X_i]$, which represents the propensity score $P(D_i=1|X_i)$ and is typically framed as a classification task when treatment is binary.
...
Having established the general framework and the techniques that ensure valid inference, we now turn to adapting this approach for Difference-in-Differences settings.
...
This section presents two applications of DML to DiD designs. We begin with the canonical two-period case where all treated units receive treatment at the same time. We then extend the framework to handle staggered treatment adoption, where different units may begin treatment at different time periods, addressing the complications this introduces for estimation and aggregation.
...
Here, $G_i = g$ identifies units in cohort $g$ (those first treated at time $g$), $C = 1$ identifies control units (never-treated or not-yet-treated at time $t$), and $n_g$ denotes the cohort size. The notation can be simplified by defining $\hat{\mu}_0(X_i, t) = E[Y_t-Y_1|D=0,X]$ and $\hat{p}_g(X_i) = E[D=1|X]$.
