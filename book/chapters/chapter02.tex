...
We begin with the classical Difference in Differences design, lay out identification and estimation in the two period case, move to extensions for staggered adoption, and clarify the role of covariates.

\section{The DiD Estimator}

We first define the canonical two group, two period estimator and link it to the Average Treatment Effect on the Treated.
...
Next, we state the identifying requirement and illustrate it through an algebraic decomposition and parallel trends plots.
...
In this decomposition, we can see that the first term corresponds to the ATT estimator. Please note that the superscripts denote whether the group corresponds to the treated ($t$) or control ($c$), and whether it was treated (1) or not (0).

But the second and third terms cancel out if the parallel trends assumption holds, basically because if the group that received the treatment and the group that didn't receive the treatment would both behave the same in the absence of treatment, then both would be equal before and after the treatment. So the terms would cancel out, and we would only have the ATT.
...
Staggered adoption complicates identification and motivates alternative estimators. We begin with the standard two way fixed effects specification and then summarize recent advances that address its limitations.
...
To address these issues, alternative estimators have been developed by different authors. However, in this paper, we will be focusing on the proposal from \cite{callway_santana_2021}, who propose a reliable way to estimate staggered DiD. In sum, while TWFE is convenient, it can be problematic under staggered designs with heterogeneous effects.

\section{Extensions to Covariates}

Finally, we motivate conditioning on covariates and clarify when and how to include them in a Difference in Differences design.
